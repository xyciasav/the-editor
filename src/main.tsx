  ...
  return <main>
    ...
    <section className="content">
      ...
      {!shoot ? <div className="hero">
        ...
        {created && <div className={created.previews.length ? "notice success" : "notice error"}><b>{created.previews.length ? "Shoot created" : "Preview generation failed"}</b><span>{created.previews.length} previews generated{created.failures.length ? ` · ${created.failures.length} could not be processed` : ""}. Originals were not changed.{created.failures[0] ? ` First error: ${created.failures[0].message}` : ""}</span></div>}
